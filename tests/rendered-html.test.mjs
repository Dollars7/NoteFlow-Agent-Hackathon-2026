...
test("implements dual auth, unified notes, scoped scheduling, and private persistence", async () => {
  ...
  assert.match(engine, /goalRelevance: number/);
  assert.doesNotMatch(engine, /type GoalId|baseGoal|export const goals/);
...
test("defaults to English, keeps Chinese in place, and reviews Agent plans before practice", async () => {
  ...
  assert.doesNotMatch(hackathonDemo, /Add reminder to calendar/);
  assert.doesNotMatch(hackathonDemo, /Notification\.requestPermission/);
  assert.match(hackathonDemo, /extractExplicitPlanningSignals/);
  assert.match(hackathonDemo, /Daily time budget/);
  assert.match(hackathonDemo, /When should the plan begin/);
  assert.match(hackathonDemo, /projectKnowledgeAreas/);
  assert.match(hackathonDemo, /hackathonHandoffKey/);
  assert.match(hackathonDemo, /extractNextRetrievalPrompts/);
  ...
  assert.match(clientApp, /handoff\.nextRetrievalPrompts/);
  ...
  assert.match(clientApp, /Prepare the notes this plan will use/);
  assert.match(clientApp, /Open Notes/);
  assert.match(clientApp, /projectSkillsForHandoff/);
  assert.match(clientApp, /mastery: 0\.3/);
  assert.match(clientApp, /retention: 0\.3/);
  assert.match(clientApp, /"agent-selected", projectTag/);
  assert.match(clientApp, /index === 0/);
  assert.doesNotMatch(clientApp, /skillId: "ood"/);
  assert.match(clientApp, /Notification\.requestPermission/);
  assert.match(clientApp, /setPhase\("pre"\)/);
  assert.match(clientApp, /setPhase\("idle"\)/);
  assert.match(clientApp, /Plan set · start when you are ready/);
  assert.match(clientApp, /Add to calendar/);
  ...
  assert.match(goalPlanner, /skills\.map/);
  assert.doesNotMatch(goalPlanner, /import \{ skillScopes/);
  assert.match(goalPlanner, /When should learning start/);
  assert.doesNotMatch(goalPlanner, /Add this time range to calendar/);
  assert.match(goalPlanner, /disabled=\{!scheduleConfirmed\}/);
  ...
  assert.match(handoff, /type LearningProject/);
  assert.match(handoff, /extractExplicitPlanningSignals/);
  ...
  assert.match(handoff, /nextRetrievalPrompts: string\[\]/);
  assert.match(handoff, /extractNextRetrievalPrompts/);
